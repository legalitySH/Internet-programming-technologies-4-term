...
import by.shymko.second.SAX.ManagerSAX;
import by.shymko.second.Serialization.Serializator;
...
public class Main {
    ...
    public static void main(String[] args) throws PriceFormatException, NullPageException, EmptyNameException, YearFormatException, IssueFormatException, EditionFindException, IOException, ParserConfigurationException, SAXException {
        ...
        try
        {
            ...
            ManagerSAX manager = new ManagerSAX("files/Book.xml", "xsd/Book.xsd",logger);
            manager.validSchema();
            List<Book> bookList = manager.parseBooks("files/Book.xml");

            for (Book book : bookList) {
                store.add(book);
            }

            Serializator serializator = new Serializator(logger);
            serializator.serializeObject(bookList.getFirst() , "data/book.dat");
            Book deserializedBook = serializator.deserializeObject("data/book.dat");
            System.out.println("Десериализация - " + deserializedBook);

            List<Book> bookResult = bookList.stream()
                    .filter(book -> book.getGenre() == Genre.FICTION)
                    .collect(Collectors.toList());

            System.out.println("Books with FICTION genre");
